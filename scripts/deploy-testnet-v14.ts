...
async function main() {
  ...
  console.log("Deploying (v14: real Uniswap V4 Universal Router integration");
  console.log("  -- replaces the entire V3-style swap path. Real liquidity on");
  console.log("  Robinhood Chain mainnet lives on V4, not V3 -- confirmed via");
  console.log("  direct on-chain research. Swap encoding confirmed correct by");
  console.log("  decoding an actual successful mainnet transaction's real");
  console.log("  calldata, not assumed from docs. Also fixes: Permit2 is now");
  console.log("  constructor-injected (was hardcoded to the mainnet canonical");
  console.log("  address, silently breaking any non-mainnet deployment); Permit2");
  console.log("  allowances are now revoked after every swap (adversarial review");
  console.log("  fix); TAKE_ALL now matches the real confirmed mainnet pattern.)");
  ...
  console.log("  StaxVault (v14):", await vault.getAddress());
  ...
  console.log("\n=== V14 DEPLOYMENT COMPLETE ===");
  console.log("NEW StaxVault address:", await vault.getAddress());
  console.log("Changes in this version vs v13:");
