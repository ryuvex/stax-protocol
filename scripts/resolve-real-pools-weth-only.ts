...
function parseScanFile(path: string): Candidate[] {
  ...
  for (const line of lines) {
    ...
    // THE FIX: only keep candidates paired against WETH or native ETH.
    // Everything else (SPY, other stock tokens, random junk tokens) is
    // structurally unusable by the vault's swap functions and is
    // excluded here, before any liquidity ranking happens.
    const isWeth = pairedWith.toLowerCase() === WETH.toLowerCase() || pairedWith === "WETH";
    const isNativeEth = pairedWith === "NATIVE ETH";
    if (!isWeth && !isNativeEth) continue;
...
async function main() {
  ...
  console.log(`Parsed ${candidates.length} WETH/native-ETH-paired candidate entries (all other pairings excluded).\n`);
  ...
  for (const symbol of TICKERS) {
    ...
    if (uniqueCandidates.length === 0) {
      console.log(`  ${symbol.padEnd(6)} -- ZERO WETH/native-ETH-paired candidates found. No usable pool exists for this ticker under the vault's actual swap constraints.`);
      results[symbol] = [];
      continue;
    }
    ...
    for (const c of uniqueCandidates) {
      const otherAddress = c.pairedWith === "NATIVE ETH" ? ethers.ZeroAddress : WETH;
    ...
    try {
      ...
      if (ranked.length === 0) {
        console.log(`  ${symbol.padEnd(6)} -- all WETH-paired candidates used a blocked/unverified hook.`);
        continue;
      }

      const top = ranked[0];
      console.log(
        `  ${symbol.padEnd(6)} -- ${ranked.length} usable WETH/native-ETH candidate(s). Best: liquidity=${top.liquidity} fee=${top.candidate.fee} tickSpacing=${top.candidate.tickSpacing} hooks=${top.candidate.hooks}`
      );
    } catch (err: any) {
  ...
  console.log("\n=== FINAL: BEST WETH/NATIVE-ETH-PAIRED POOL PER TICKER ===\n");
  for (const symbol of TICKERS) {
